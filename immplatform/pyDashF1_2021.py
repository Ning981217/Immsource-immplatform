...
from f1_2021_telemetry.packets import *
from setting_define import *
import datetime

class pyDashF1_2021:

    def __init__(self, f1_2021_pid, dash, game_data, port, enable):
        self.f1_2021_pid = f1_2021_pid
        ...
        try:
            ...
            if pid_exists(self.f1_2021_pid):
                self.status = True
                self.game_data.t_game_id = game_id['f12021']
        except Exception as e:
            print('pyDashF1-2021 error', e)
            try:
    ...
    def _update(self):
        if pid_exists(self.f1_2021_pid) and self.status == True and self.enable:
            try:
                while not self.close:
                    ...
                    if isinstance(packet, PacketCarTelemetryData):
                        ...
                        if data_packet.surfaceType[2] == 1:
                            self.dash.load_diff = self.game_data.t_speed / 0.5
                        else:
                            if data_packet.surfaceType[3] == 1:
                                self.dash.load_diff = -self.game_data.t_speed / 0.5
                            else:
                    ...
                    if isinstance(packet, PacketCarSetupData):
                    ...
                    if isinstance(packet, PacketCarStatusData):
                    ...
                    if not pid_exists(self.f1_2021_pid):
                        self.status = False
                        try:
                            self.server_socket.shutdown(0)
                            self.server_socket.shutdown(1)
                ...
                print('f1 2021 dash updata error', e)
                self.status = False
                try:
                    self.server_socket.shutdown(0)
                    self.server_socket.shutdown(1)
