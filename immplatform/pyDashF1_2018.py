...
from f1_2018_telemetry.packets import *
from setting_define import *
import datetime

class pyDashF1_2018:

    def __init__(self, f1_2018_pid, dash, game_data, port, enable):
        self.f1_2018_pid = f1_2018_pid
        ...
        try:
            ...
            if pid_exists(self.f1_2018_pid):
                self.status = True
                self.game_data.t_game_id = game_id['f12018']
        except Exception as e:
            print('pyDashF1-2018 error', e)
    ...
    def _update(self):
        if pid_exists(self.f1_2018_pid) and self.status == True and self.enable:
            print('f12018')
            try:
                while not self.close:
                    ...
                    if not pid_exists(self.f1_2018_pid):
                        self.status = False
                        try:
                ...
                print('f12018 dash updata error', e)
                self.status = False
                try:
                    self.server_socket.shutdown(0)
                    self.server_socket.shutdown(1)
